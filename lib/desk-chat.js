...
const { answerCommand, controlPatch, needsExecution } = require("./mind");
...
async function readControl() {
  try {
    return JSON.parse(await getKey("control") || "null");
  } catch {
    return null;
  }
}

async function beat(source) {
  await putKey("heartbeat", JSON.stringify({ at: new Date().toISOString(), source }));
}

async function localAwake(withinMs = 3 * 60 * 1000) {
  const auth = token();
  if (!auth) return "unknown";
  let response;
  try {
    response = await fetch(url("heartbeat"), { headers: { Authorization: `Bearer ${auth}` } });
  } catch {
    return "unknown";
  }
  ...
  try {
    ...
    const fresh = body.source === "local" && Number.isFinite(at) && Date.now() - at < withinMs;
...
async function writeControl(patch) {
  ...
  await putKey("control", JSON.stringify({ ...current, ...patch }));
}
...
async function enqueue(text) {
  ...
  const reply = answerCommand(text);
  chat.push({ at, role: "user", text });
  chat.push({ at, role: "desk", text: reply });
  const patch = controlPatch(text);
  if (patch) await writeControl(patch);
...
module.exports = { readChat, enqueue, takeInbox, reply, readControl, writeControl, putRaw: putKey, getRaw: getKey, beat, localAwake };
