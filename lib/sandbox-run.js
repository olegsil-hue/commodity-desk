...
function buildPlan({ brentDay, cash, lines, roles, prev, oilLast, book, allowOilShort, enabled }) {
  ...
  const learned = learn.candidates().slice(0, 3).map((item) => `${item.ticker} (у крупных счетов около +${item.avg.toFixed(1).replace(".", ",")}%)`).join(", ");
  const compare = learned || "акции из книги";
  ...
  if (!enabled) {
    ...
    morning = [
      `В 9:00 сделаю так. Свободно ${free}, ${heldText}. До открытия заявок нет.`,
      `1. Возьму цены открытия и заново посчитаю, насколько каждая бумага ушла от вчерашнего закрытия.`,
      `2. Сравню BR-10.26 с ${compare}.`,
      "3. Куплю одну самую сильную. Контракт нефти куплю, только если его ход больше, чем у этих бумаг, и свободно не меньше 20 000 ₽.",
      "4. Если после первой покупки останется больше 8 000 ₽, могу взять вторую бумагу.",
      "5. Дальше продаю лонг около +1% от входа и закрываю по стопу около −1%. Отскок закрываю около +0,6% и −0,6%.",
    ].join("\n");
  }
  return {
    day: morning,
...
async function step() {
  ...
  try {
    ...
    if (!state.enabled) {
      ...
      saveState(state);
      cache = { ...shot, ...state };
      await deskChat.beat(process.env.SESSION_SLOT === "tick" ? "github" : "local").catch(() => {});
      return cache;
    }
